...
You (the agent) are reading this because someone ran `agentfront cli cite` against
...
6. Run `agentfront cli doctor .` from the host project to confirm the seven
   rubric bundles pass.

## Rubric bundles checked by `agentfront cli doctor`
...
> (bundle 7) to pass the full rubric. Use agentfront's own implementations as
> templates: `agentfront explain overview` and `agentfront explain doctor` describe the
> contract; the source under `agentfront/overview/` and `agentfront/doctor/` shows the
> shape. (Tracked: ship `overview.py` and `doctor.py` reference templates
> in a follow-up cite refresh.)

## After integration

Delete this reference (`rm -rf .agentfront/reference/`) or re-run `agentfront cli cite` to
refresh it. The `.agentfront/` entry in `.gitignore` keeps it out of commits.

## Rubric audit verb

Run `agentfront cli doctor .` from the host project to confirm the seven rubric
bundles pass. `agentfront cli verify` is a deprecated alias for the same command
