"""``agentfront doctor`` and ``agentfront cli doctor`` — diagnosis with actionable remediation.

The doctor verb is the agent-first contract for "tell me what's wrong and
how to fix it." Two entry points:

* ``agentfront doctor [path]`` — global. With no path, runs the in-process
  self-diagnosis from :mod:`agentfront.doctor`. With a path, delegates to the
  target-audit flow (same as ``agentfront cli doctor <path>``).
* ``agentfront cli doctor <path>`` — replaces ``agentfront cli verify``: runs the rubric
  against the target. Adds ``--fix`` / ``--dry-run`` for auto-remediation
  of checks that declare ``auto_fixable=true``. ``agentfront cli verify`` stays
...
# `agentfront doctor` is the default; `cmd_cli_doctor` overrides with
# "agentfront cli doctor" so diagnostics from the noun-scoped form name the verb
...
def _self_headline(summary: dict[str, int], healthy: bool) -> str:
    ...
    # actually working with. See agentculture/agentfront#13.
    suffix = "Run 'agentfront doctor <path>' to audit a target CLI."
    if healthy:
        return (
            f"agentfront doctor: structural self-check passed "
            f"({summary['passed']}/{summary['total']}). {suffix}"
        )
    return (
        f"agentfront doctor: structural self-check failed "
...
def _resolve_tool_name(target_path: Path, *, command: str = _DEFAULT_DOCTOR_COMMAND) -> str:
    ...
    ``command`` is the verb the user invoked (``agentfront doctor`` or
    ``agentfront cli doctor``); it is woven into the not-a-project-root remediation
    ...
    if parsed.scheme != "file":
        raise AgentfrontError(
            ...
            message=(
                ...
                f"agentfront doctor needs a source checkout with pyproject.toml"
            ),
            remediation=(
    ...
    if not (isinstance(dir_info, dict) and dir_info.get("editable") is True):
        raise AgentfrontError(
            ...
            message=(
                ...
                f"agentfront doctor needs an editable install so source mutations are visible"
            ),
            remediation=(
...
def _resolve_target_or_raise(
    ...
    * ``raw_path`` set → resolve as a filesystem path.
    ...
    for ``agentfront doctor`` vs. ``agentfront cli doctor``). ``command`` is the verb the
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    if raw is None and package is None:
        ...
        if fix or dry_run:
            emit_diagnostic(
                ...
                "use them with `agentfront cli doctor <path>`."
            )
    ...
    # Target audit (alias for `agentfront cli doctor <path>` / `--package <name>`).
...
def cmd_cli_doctor(args: argparse.Namespace) -> int:
    """Handler for ``agentfront cli doctor [path] | --package <name>``.
    ...
    raw = getattr(args, "path", None)
    package = getattr(args, "package", None)
    # `agentfront cli doctor` (no args) audits cwd. We can't tell "user typed `.`"
    ...
    if raw is None and package is None:
        ...
        target_path = _resolve_target_or_raise(
            raw_path=raw, package=package, command="agentfront cli doctor"
        )

    tool_name, results = _run_target_audit(
        target_path, fix=fix, dry_run=dry_run, command="agentfront cli doctor"
    )
    _emit_payload(
        subject=str(target_path),
        tool=tool_name,
...
def cmd_cli_verify_deprecated(args: argparse.Namespace) -> int:
    """Handler for the deprecated ``agentfront cli verify`` alias.

    Emits a deprecation diagnostic to stderr, then forwards to
    :func:`cmd_cli_doctor`. Removed in v0.6.0.
    """
    emit_diagnostic(
        "deprecated: 'agentfront cli verify' is an alias for 'agentfront cli doctor'; "
...
def register(sub: argparse._SubParsersAction) -> None:
    """Register the global ``agentfront doctor`` verb. Called from agentfront.cli.__init__."""
    p = sub.add_parser(
        "doctor",
        help=(
            "Diagnose agentfront (no path) or audit a target CLI (with path) and "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "--package) to self-diagnose agentfront's own install."
        ),
    )
    p.add_argument(
        "--package",
