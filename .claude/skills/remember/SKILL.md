...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope agentfront
...
scope:** the default personal scope is **private** (`--scope agentfront
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `agentfront`). Private records are served only to a recall in the same
