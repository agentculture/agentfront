"""Fix registry for ``afi doctor --fix``.
...
because the safe auto-fixes (e.g., re-citing a stale ``.afi/reference/``
tree on a target) live next to ``afi cli cite``, not next to the rubric.
